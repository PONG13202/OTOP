...
if (!isset($_SESSION['user_id']) || empty($_SESSION['user_id'])) {
    header("Location: login.php"); // เปลี่ยนจาก index.php เป็น login.php เพื่อให้เหมาะสมกับหน้าบ้าน
    exit();
}
...
if (!$user) {
    header("Location: index.php"); // เปลี่ยนจาก index.php เป็น login.php
    exit();
}
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        if (empty($new_pwd) && empty($old_pwd) || (!empty($new_pwd) && !empty($old_pwd) && password_verify($old_pwd, $user['user_pwd']))) {
            ...
            if (mysqli_stmt_execute($update_stmt)) {
                echo "<script>alert('อัปเดตข้อมูลเรียบร้อยแล้ว'); window.location.href = 'index.php';</script>"; // เปลี่ยนจาก back_office.php เป็น profile.php
            } else {
