...
// รับค่า trd_id จากพารามิเตอร์ใน URL
$trd_id = isset($_GET['trd_id']) ? (int)$_GET['trd_id'] : 0;

// ตรวจสอบว่ามี trd_id หรือไม่ ถ้าไม่มีจะรีไดเรกต์หรือแสดงข้อความผิดพลาดได้
if ($trd_id === 0) {
...
<head>
  ...
  <title>ข้อมูลประเพณี - พระนครศรีอยุธยา</title>
...
<body>
 ...
 <button class="carousel-control-next" type="button" data-bs-target="#bannerCarousel" data-bs-slide="next">
  ...
  <!-- ปุ่มย้อนกลับ -->
  <div class="container">
    <a href="tradition.php" class="btn btn-outline-primary mb-3">
      <i class="fas fa-arrow-left"></i> ย้อนกลับ
    </a>
  </div>
  <div class="container">
    <?php
    // ดึงข้อมูลประเพณีที่ต้องการโดยระบุ trd_id
    $sql = "
            SELECT t.trd_id, t.trd_name, t.trd_detail,
                   GROUP_CONCAT(ti.trd_img_name) AS images
              FROM tradition t
              LEFT JOIN tradition_img ti 
                     ON t.trd_id = ti.trd_id
             WHERE t.trd_show = 1
               AND t.trd_id = $trd_id
             GROUP BY t.trd_id
             LIMIT 1
    ...
    if (!$data) {
      echo '<h2 class="section-title my-5">ไม่พบข้อมูลประเพณี</h2>';
    } else {
      $trd_name = htmlspecialchars($data['trd_name']);
      $trd_detail = $data['trd_detail'];
    ...
    ?>
      ...
      <h1 class="section-title my-5"><?php echo $trd_name; ?></h1>

      <!-- ถ้ามีหลายรูป สร้างคารูเซล หรือโชว์เป็น Thumbnails -->
      <?php if (!empty($images_array)): ?>
        <!-- Carousel หลายรูป -->
        <div id="traditionCarousel" class="carousel slide mb-3" data-bs-ride="carousel">
          <div class="carousel-inner">
            <?php
            $first_active = "active";
          ...
          <button class="carousel-control-prev" type="button" data-bs-target="#traditionCarousel" data-bs-slide="prev">
            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
          </button>
          <button class="carousel-control-next" type="button" data-bs-target="#traditionCarousel" data-bs-slide="next">
            <span class="carousel-control-next-icon" aria-hidden="true"></span>
          </button>
        </div>
      ...
      <div class="detail-content mb-5">
        <h4 class="detail-title">รายละเอียดประเพณี</h4>
        <p><?php echo nl2br(htmlspecialchars($trd_detail)); ?></p>
      </div>
  ...
  <script>
    function showImageInCarousel(src) {
      ...
      const items = document.querySelectorAll('#traditionCarousel .carousel-item');
      let indexToShow = 0;
      ...
      const carousel = new bootstrap.Carousel('#traditionCarousel');
