...
// ตรวจสอบบทบาท Super Admin
if ($_SESSION['role'] == 'role_admin') {
    $sql_check_role = "SELECT role_sup.sup_id FROM role_sup WHERE user_id = ?";
    $stmt_check = mysqli_prepare($project_connect, $sql_check_role);
    mysqli_stmt_bind_param($stmt_check, "i", $user_id);
    mysqli_stmt_execute($stmt_check);
    $result_check = mysqli_stmt_get_result($stmt_check);

    if (mysqli_num_rows($result_check) > 0 && $_SESSION['user_id'] != $user_id) {
        echo "<script>alert('ไม่สามารถแก้ไขได้: ไม่สามารถแก้ไขข้อมูลของผู้ใช้ที่เป็น Super Admin'); window.location.href = 'user_show.php';</script>";
        exit();
    }
...
// ตรวจสอบบทบาท Admin
if ($_SESSION['role'] == 'role_admin') {
    $sql_check_role = "SELECT role_admin.admin_id FROM role_admin WHERE user_id = ?";
    $stmt_check = mysqli_prepare($project_connect, $sql_check_role);
    mysqli_stmt_bind_param($stmt_check, "i", $user_id);
    mysqli_stmt_execute($stmt_check);
    $result_check = mysqli_stmt_get_result($stmt_check);
    ...
    if (mysqli_num_rows($result_check) > 0 && $_SESSION['user_id'] != $user_id) {
        echo "<script>alert('ไม่สามารถแก้ไขได้: ไม่สามารถแก้ไขข้อมูลของผู้ใช้ที่เป็น Admin คนอื่นได้'); window.location.href = 'user_show.php';</script>";
        exit();
    }
    mysqli_stmt_close($stmt_check);
}
...
if (isset($_POST["cmd_ok"])) {
    ...
    $user_pwd = !empty($_POST['user_pwd']) ? password_hash($_POST['user_pwd'], PASSWORD_DEFAULT) : $user['user_pwd'];
    ...
    if (mysqli_num_rows($check_result) > 0) {
            ...
            if ($check !== false) {
                ...
                if ($_FILES["user_img"]["size"] <= 20000000 && in_array($imageFileType, ['jpg', 'png', 'jpeg', 'gif'])) {
                    ...
                    if (move_uploaded_file($_FILES["user_img"]["tmp_name"], $target_file)) {
                        ...
                        echo "<script>alert('ไม่สามารถย้ายไฟล์ได้ กรุณาตรวจสอบสิทธิ์ของโฟลเดอร์');</script>";
                    }
                } else {
        ...
        // อัปเดตข้อมูล
        $update_sql = "UPDATE user SET user_name=?, user_fname=?, user_lname=?, user_email=?, user_addr=?, user_phone=?, user_pwd=?, user_img=?, user_show=? WHERE user_id=?";
        $update_stmt = mysqli_prepare($project_connect, $update_sql);
        mysqli_stmt_bind_param($update_stmt, "sssssssssi", $user_name, $user_fname, $user_lname, $user_email, $user_addr, $user_phone, $user_pwd, $user_img, $user_show, $user_id);
        if (mysqli_stmt_execute($update_stmt)) {
            header("Location: user_show.php");
            exit();
        } else {
            echo "<script>alert('เกิดข้อผิดพลาดในการอัปเดตข้อมูล: " . mysqli_stmt_error($update_stmt) . "');</script>";
        }
...
<body>
    <div class="main-content">
        <div class="form-card">
            ...
            <h2 class="form-title text-center"><i class="fas fa-user-edit me-2"></i>แก้ไขข้อมูลสมาชิก</h2>
            ...
            <form method="POST" enctype="multipart/form-data" class="row g-4">
                ...
                <div class="col-12">
                    <div class="input-group">
                        ...
                        <select class="form-select"
                            ...
                            name="user_show"
                            required>
                            <option value="1" <?php echo $user['user_show'] == 1 ? 'selected' : ''; ?>>แสดง</option>
                            <option value="0" <?php echo $user['user_show'] == 0 ? 'selected' : ''; ?>>ไม่แสดง</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="col-12 text-center mt-4">
                    <button type="submit"
                        ...
                        <i class="fas fa-user-edit me-2"></i>แก้ไขสมาชิก
                    </button>
                </div>
            </form>
        </div>
